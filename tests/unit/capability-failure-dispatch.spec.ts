/**
 * Unit tests for the failed-capability → inbox fan-out
 * (src/dashboard/lib/push/capability-failure-dispatch.ts). A capability that fails on a
 ...
 *   - `dispatchCapabilityFailures`: the orchestration guards — ignore non-routable
...
function rec(over: Partial<CompanyActivityRecord> = {}): CompanyActivityRecord {
  return {
    ...
    action: "Ran capability: Verify changelog",
    capability: "changelog-verify",
    capabilityTitle: "Verify changelog",
    agent: "qa-engineer",
...
describe("buildEntries", () => {
  it("emits one entry per (operator × failure) with a deterministic id", () => {
    const failure = rec({ capability: "d1", ts: "2026-05-23T00:00:00.000Z" });
    ...
    expect(entries.map((e) => e.id)).toEqual([
      "capability-fail:alice:d1:2026-05-23T00:00:00.000Z",
      "capability-fail:bob:d1:2026-05-23T00:00:00.000Z",
    ]);
    expect(entries[0]).toMatchObject({
      ...
      threadType: "Run",
      title: "Capability failed: Verify changelog",
  ...
  it("labels stopped-early kinds differently from a real failure", () => {
    ...
    expect(entries[0].title).toBe("Capability stopped early: Verify changelog");
  ...
  it("keeps the generic 'failed' wording for older records (no kind)", () => {
    ...
    expect(entries[0].title).toBe("Capability failed: Verify changelog");
...
describe("dispatchCapabilityFailures", () => {
  it("ignores non-push events", async () => {
    await dispatchCapabilityFailures("issue_comment", { action: "created" });
  ...
  it("ignores pushes without activity state paths", async () => {
    await dispatchCapabilityFailures(
      "push",
  ...
  it("appends one entry per operator for each failed record", async () => {
    h.fetchCompanyActivity.mockResolvedValue([
      rec({ capability: "d1" }),
      rec({ capability: "d2", outcome: "completed" }),
    ]);
    await dispatchCapabilityFailures(
      "push",
      pushEvent("refs/heads/kody-state", ["widgets/activity/2026-05-23.jsonl"]),
