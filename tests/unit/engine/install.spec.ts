...
import { installEngine, type InstallEngineInput } from '@dashboard/lib/engine/install'
...
function createMockOctokit(overrides?: Record<string, unknown>) {
  return {
      ...
      repos: {
        ...
        createOrUpdateFileContents: vi.fn().mockResolvedValue({
          data: {
            ...
            content: { html_url: 'https://github.com/example/repo/blob/main/.github/workflows/kody.yml' },
          },
        }),
      },
      actions: {
    ...
    ...overrides,
  } as unknown as ImportType<'@octokit/rest'>['prototype']
}
  ...
  vi.spyOn(octokit.rest.repos, 'createOrUpdateFileContents').mockImplementation(
    async (params: { path: string; content: string }) => {
      ...
      return {
        data: {
          commit: { sha: 'commitsha' },
          content: { html_url: `https://github.com/example/repo/blob/main/${params.path}` },
        },
      }
    },
  )
...
describe('installEngine', () => {
  describe('kody.config.json creation', () => {
    ...
    it('kody.config.json contains model.default when variables.json exists with models', async () => {
      ...
      vi.spyOn(octokit.rest.repos, 'getContent').mockImplementation(async (params: { path: string }) => {
        if (params.path === '.kody/variables.json') {
          const variablesContent = JSON.stringify({
            LLM_MODELS: [
              { id: 'google/gemini-2.5-flash', label: 'Gemini 2.5 Flash', provider: 'google', protocol: 'openai', baseURL: '', modelName: 'gemini-2.5-flash', apiKeySecret: 'GEMINI_API_KEY', enabled: true, default: true },
            ],
          })
          return {
            data: {
