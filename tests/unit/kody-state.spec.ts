...
const minimalState: KodyTaskState = {
  ...
  core: {
    ...
    lastOutcome: { type: 'PLAN_STARTED', payload: {}, timestamp: '2026-05-10T14:00:00Z' },
    attempts: { plan: 1 },
  },
}
...
describe('parseKodyStateComment', () => {
  ...
  it('returns null when only the begin marker is present', () => {
    expect(parseKodyStateComment(`Some text\n${STATE_BEGIN}\n\`\`\`json\n{}\n\`\`\``)).toBeNull()
  })
...
describe('findKodyStateInComments', () => {
  ...
  it('prefers the most recent state comment when multiple are present', () => {
    ...
    const result = findKodyStateInComments([{ body: older }, { body: 'noise' }, { body: newer }])
