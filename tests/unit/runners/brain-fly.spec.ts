...
describe('brainAppName', () => {
  ...
  it('replaces non-alphanumerics with single hyphens', () => {
    expect(brainAppName('user.name_with-stuff')).toBe('kody-brain-user-name-with-stuff')
  })
...
describe('flyFetch', () => {
  ...
  it('throws on 404 when allow404 is unset', async () => {
    installFetchStub(() => ({ status: 404, json: { error: 'not found' } }))
    await expect(flyFetch('/apps/foo', { token: TOKEN })).rejects.toThrow(/404/)
  })

  it('throws on 5xx with status code in the message', async () => {
    installFetchStub(() => ({ status: 503, json: { error: 'service unavailable' } }))
    await expect(flyFetch('/apps/foo', { token: TOKEN })).rejects.toThrow(/503/)
  })
...
describe('provisionBrain', () => {
  it('creates the app and machine when neither exists', async () => {
    const calls = installFetchStub((call) => {
      if (call.url.endsWith('/apps/kody-brain-alice') && call.method === 'GET') {
        return { status: 404 }
      }
      if (call.url.endsWith('/apps') && call.method === 'POST') {
        return { json: { name: 'kody-brain-alice' } }
      }
      if (call.url.endsWith('/apps/kody-brain-alice/machines') && call.method === 'GET') {
        return { status: 404 }
      }
      if (call.url.endsWith('/apps/kody-brain-alice/machines') && call.method === 'POST') {
    ...
    const machineCreate = calls.find(
      (c) => c.method === 'POST' && c.url.endsWith('/apps/kody-brain-alice/machines'),
    )!
    const cfg = (machineCreate.body as { config: { env: Record<string, string> } }).config
  ...
  it('throws when an existing machine has no BRAIN_API_KEY (corrupted state)', async () => {
    installFetchStub((call) => {
      ...
      if (call.url.endsWith('/apps/kody-brain-alice/machines') && call.method === 'GET') {
        return { json: [{ id: 'm-bad', state: 'started', config: { env: {} } }] }
      }
  ...
  it('passes the perf tier through to the Fly guest shape', async () => {
    ...
    const guest = (create.body as { config: { guest: { cpus: number; memory_mb: number } } })
      .config.guest
  ...
  it('stringifies allSecrets into ALL_SECRETS env', async () => {
    const calls = installFetchStub((call) => {
      ...
      if (call.method === 'GET' && call.url.endsWith('/machines')) return { json: [] }
      if (call.method === 'POST' && call.url.endsWith('/machines')) {
    ...
    const env = (create.body as { config: { env: Record<string, string> } }).config.env
...
describe('destroyBrain', () => {
  ...
  it('issues DELETE on the app when it exists', async () => {
    const calls = installFetchStub((call) => {
      ...
      if (call.method === 'DELETE' && call.url.includes('/apps/kody-brain-alice')) {
...
describe('brainStatus', () => {
  ...
  it('rejects when flyToken is empty', async () => {
    await expect(
      brainStatus({ flyToken: '', owner: 'alice' }),
    ).rejects.toThrow(/flyToken required/)
  })
})
