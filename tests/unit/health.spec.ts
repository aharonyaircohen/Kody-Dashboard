...
import {
  ...
  orderSignals,
  countByLevel,
} from "@dashboard/lib/health/rollup";
import {
...
import {
  keyNameForModelSpec,
  buildModelSignal,
} from "@dashboard/lib/health/model-health";
...
import {
  buildRunsSignal,
  type RunLite,
} from "@dashboard/lib/health/runs-health";
import {
...
describe("rollup", () => {
  it("worst level wins", () => {
    ...
    expect(rollupLevel([sig("a", "ok"), sig("b", "degraded")])).toBe(
      "degraded",
    );
  ...
  it("counts by level", () => {
    expect(
      countByLevel([sig("a", "down"), sig("b", "down"), sig("c", "ok")]),
    ).toEqual({
      ok: 1,
      degraded: 0,
      down: 2,
...
describe("github actions status mapping", () => {
  it("maps the statuspage vocabulary", () => {
    ...
    expect(mapActionsComponentStatus("degraded_performance").level).toBe(
      "degraded",
    );
    ...
    expect(mapActionsComponentStatus("under_maintenance").level).toBe(
      "degraded",
    );
  ...
  it("finds the Actions component case-insensitively", () => {
    ...
    expect(
      findActionsComponent([{ name: "actions", status: "major_outage" }])
        ?.status,
    ).toBe("major_outage");
...
describe("token rate-limit classification", () => {
  ...
  it("flags the kodyade 60/hr case as down", () => {
    expect(classifyRateLimit({ limit: 60, remaining: 46, reset }).level).toBe(
      "down",
    );
  });
  it("exhausted window is down", () => {
    expect(classifyRateLimit({ limit: 5000, remaining: 0, reset }).level).toBe(
      "down",
    );
  });
  it("low remaining is degraded", () => {
    expect(classifyRateLimit({ limit: 5000, remaining: 50, reset }).level).toBe(
      "degraded",
    );
  });
  it("healthy is ok", () => {
    expect(
      classifyRateLimit({ limit: 5000, remaining: 4900, reset }).level,
    ).toBe("ok");
  });
});
...
describe("model key resolution", () => {
  it("uses preset keyHint where known", () => {
    expect(keyNameForModelSpec("anthropic/claude")?.keyName).toBe(
      "ANTHROPIC_API_KEY",
    );
    expect(keyNameForModelSpec("google/gemini-2.0")?.keyName).toBe(
      "GEMINI_API_KEY",
    );
  });
  ...
  it("buildModelSignal: missing key is down, present is ok, unset is degraded", () => {
    expect(buildModelSignal({ modelSpec: null, hasKey: false }).level).toBe(
      "degraded",
    );
    expect(
      buildModelSignal({ modelSpec: "minimax/x", hasKey: false }).level,
    ).toBe("down");
    expect(
      buildModelSignal({ modelSpec: "minimax/x", hasKey: true }).level,
    ).toBe("ok");
  });
});
...
describe("vault signal", () => {
  it("unconfigured and no-token are degraded; full is ok", () => {
    expect(
      buildVaultSignal({ configured: false, hasGithubToken: false }).level,
    ).toBe("degraded");
    expect(
...
describe("webhook delivery classification", () => {
  ...
  it("all failed is down", () => {
    expect(
      classifyDeliveries(true, [{ status_code: 500 }, { status_code: 403 }])
        .level,
    ).toBe("down");
  });
  it("some failed is degraded", () => {
    expect(
      classifyDeliveries(true, [{ status_code: 200 }, { status_code: 500 }])
        .level,
    ).toBe("degraded");
  });
  it("all ok is ok", () => {
    expect(
      classifyDeliveries(true, [{ status_code: 200 }, { status_code: 204 }])
        .level,
    ).toBe("ok");
  });
});
...
describe("runs signal", () => {
  ...
  it("3 failures in a row is down", () => {
    expect(
      buildRunsSignal(
        [run(1, "failure"), run(5, "failure"), run(9, "failure")],
        now,
      ).level,
    ).toBe("down");
  });
  ...
  it("recent success is ok", () => {
    expect(
      buildRunsSignal([run(5, "success"), run(20, "failure")], now).level,
    ).toBe("ok");
  });
});
...
describe("dispatch failures", () => {
  ...
  it("one recent failure is degraded", () => {
    ...
    expect(buildDispatchSignal(recentDispatchFailures()).level).toBe(
      "degraded",
    );
  });
