...
async function gotoVibe(
  ...
  if (opts?.waitForTasks) {
    ...
    const firstDetail = page.locator('button[title="Open issue details"]').first()
...
test.describe('Vibe page — header', () => {
  test('refresh button kicks off a tasks refetch', async ({ page }) => {
    ...
    const respPromise = page.waitForResponse(
      (resp) =>
        resp.url().includes('/api/kody/tasks') && resp.request().method() === 'GET',
      { timeout: 8_000 },
    )
    await refresh.click()
    const resp = await respPromise.catch(() => null)
    expect(resp, 'Refresh did not trigger /api/kody/tasks fetch').not.toBeNull()
...
test.describe('Vibe page — issue list', () => {
  test('search filters rows and clear button restores them', async ({ page }) => {
    await gotoVibe(page, { waitForTasks: true })
    ...
    await expect(
      page.getByText(/no matches for/i).first(),
