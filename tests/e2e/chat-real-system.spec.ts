...
test.describe("Real chat flow @real", () => {
  ...
  test("UI send → engine reply committed to target repo within 2 min", async ({ page }) => {
    ...
    const triggerBody = JSON.parse(triggerReq.postData() ?? "{}") as { taskId?: string }
    ...
    while (Date.now() < deadline) {
      const res = await fetch(
        ...
        { headers: { accept: "application/vnd.github.v3+json", authorization: `token ${TEST_TOKEN}` } },
      )
      if (res.status === 200) {
        const data = (await res.json()) as { content?: string }
        const body = data.content ? Buffer.from(data.content, "base64").toString("utf-8") : ""
