 ...
 * @ai-summary Tiny YAML-frontmatter parser/serializer scoped to worker
 *   files. Worker markdown is allowed to start with a `---\n…\n---\n`
 *   block carrying flat scalar key/value pairs (no nesting). Today the
 *   only recognized field is `every: 15m|1h|6h|1d` for per-worker
 *   scheduling, but the parser preserves any other keys so engine-side
 *   features can read them without dashboard awareness.
 *
 *   Duplicated from jobs-frontmatter.ts so the Workers feature stays
 *   independent of Jobs — same restricted flat-YAML format, separate
 *   module.
 */
...
export type ScheduleEvery =
  ...
  | "7d"
  /**
   * Sentinel: the scheduler never auto-fires this worker. Only manual triggers
...
export interface WorkerFrontmatter {
  ...
  every?: ScheduleEvery;
  /**
   * When `true`, the scheduler skips this worker on every cron wake. Manual
 ...
 * empty `---` blocks across worker files.
 */
...
function parseFlatYaml(text: string): WorkerFrontmatter {
  const out: WorkerFrontmatter = {};
...
function serializeFlatYaml(frontmatter: WorkerFrontmatter): string[] {
  const lines: string[] = [];
  if (frontmatter.every) lines.push(`every: ${frontmatter.every}`);
  // Only emit `disabled: true` — the default (enabled) leaves the line
  // out so an unchanged worker file stays byte-identical.
