 ...
 * @ai-summary Helpers for rendering worker schedule UI: "next scheduled
 *   tick" math (driven by the engine cron in templates/kody.yml — every
 *   15 minutes) and relative-time formatting for "last ticked".
 *
 *   The engine cron is fixed at `*\/15 * * * *`. If that ever becomes
 *   per-repo configurable, expose CRON_INTERVAL_MS via the API instead
 *   of hard-coding here. Duplicated from jobs-schedule.ts.
 */
