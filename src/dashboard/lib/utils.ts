...
export function sortTasks(
  ...
  const sorted = [...tasks].sort((a, b) => {
    ...
    switch (field) {
      ...
      case "column":
        cmp =
          (COLUMN_DEFS[a.column]?.order ?? 0) -
          (COLUMN_DEFS[b.column]?.order ?? 0);
        break;
      case "riskLevel": {
      ...
      case "pipelineProgress": {
        ...
        const aCompleted = Object.values(aStages).filter(
          (s) => s.state === "completed",
        ).length;
        const bCompleted = Object.values(bStages).filter(
          (s) => s.state === "completed",
        ).length;
 ...
 */
export function getPreviewBypassUrl(
  ...
  try {
    ...
    if (raw) {
      ...
      if (
        typeof parsed.vercelBypassSecret === "string" &&
        parsed.vercelBypassSecret.trim()
      ) {
  ...
  try {
    ...
    console.warn(
      "[Kody] Invalid preview URL, cannot add bypass params:",
      previewUrl,
      error,
    );
    return previewUrl;
