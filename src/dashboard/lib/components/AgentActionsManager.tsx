...
interface AgentActionShellScript {
  name: string;
  content: string;
}
interface AgentActionSummary {
  ...
  htmlUrl: string;
  /** Legacy/profile agent field, if present. AgentResponsibilities normally own agent. */
  agent?: string | null;
  source?: "local" | "store";
  readOnly?: boolean;
}
interface AgentActionDetail extends AgentActionSummary {
...
async function listApi(
  ...
  return {
    agentActions: json.agentActions ?? [],
  };
}
...
async function readApi(
  ...
  if (!res.ok || !json.agentAction)
    throw new Error(json.message || json.error || `HTTP ${res.status}`);
  return json.agentAction;
}
...
/**
 * Standalone editor page for `/agent-actions/new` (slug=null) and
 * `/agent-actions/<slug>`. Owns the save mutation and returns to the list on
...
function AgentActionsManagerInner({
  ...
  const selected = useMemo(
    () => agentActions.find((e) => e.slug === selectedSlug) ?? null,
  ...
  // actually show "the agentAction content", so load the full record for the
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: agentActionQueryKeys.detail(
          selected?.slug ?? null,
          queryScope,
        ),
      });
...
/** One agentAction in the list — mirrors the agentResponsibility/job list row. */
function AgentActionRow({
  exec: e,
  isActive,
  onSelect,
}: {
  exec: AgentActionSummary;
  isActive: boolean;
  onSelect: () => void;
}) {
  return (
...
/** Detail pane for one agentAction — hero + actual content (instructions, model,
 * tools, skills, scripts, MCP servers). Legacy .md agentResponsibilities render the hero
 * only and link out to the file on GitHub. */
function AgentActionDetail({
  exec: e,
  detail,
  detailLoading,
  detailError,
...
function AgentActionContentBody({
  detail,
  loading,
  error,
}: {
...
function AgentActionEditorForm({
  isNew,
  initial,
  existingSlugs,
  saving,
  ...
  isNew: boolean;
  initial: AgentActionDetail | null;
  existingSlugs: Set<string>;
  saving: boolean;
  headers: Record<string, string>;
  onClose: () => void;
  ...
  const [shellScripts, setShellScripts] = useState<AgentActionShellScript[]>(
  ...
  async function analyzeToolFromSource() {
    ...
    setAnalyzing(true);
    try {
      const res = await fetch("/api/kody/agent-actions/analyze-tool", {
        method: "POST",
        headers,
  ...
  async function importSkillFromSource() {
    ...
    setImporting(true);
    try {
      const res = await fetch("/api/kody/agent-actions/import-skill", {
        method: "POST",
        headers,
        body: JSON.stringify({ source }),
      });
