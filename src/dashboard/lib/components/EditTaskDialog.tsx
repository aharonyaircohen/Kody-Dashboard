...
export function EditTaskDialog({ open, onClose, task, onSaved }: EditTaskDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const labelsWithoutPriority = labels.filter((l) => !l.startsWith('priority:'))
    const finalLabels = priority ? [...labelsWithoutPriority, priority] : labelsWithoutPriority
    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="sm:max-w-[800px] max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Update task #{task.issueNumber} on GitHub.</DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {availableLabels.map((label) => {
                const labelData = label as unknown as { name: string; color: string }
                const isSelected = labels.includes(
                  typeof labelData === 'string' ? labelData : labelData.name,
                )
                return (
                  <button
                    key={typeof labelData === 'string' ? labelData : labelData.name}
                    type="button"
                    onClick={() =>
                      toggleLabel(typeof labelData === 'string' ? labelData : labelData.name)
                    }
                    className={cn(
