...
import { getOctokit, getOwner, getRepo, invalidateMemoryCache } from './github-client'
...
function parseMemoryFile(raw: string, id: string): { meta: MemoryFrontmatter; body: string } | null {
...
async function fetchLastCommitDate(octokit: Octokit, filePath: string): Promise<string> {
  try {
...
export async function listMemoryFiles(): Promise<MemoryFile[]> {
  ...
  const ids = entries
    .filter((e) => e.type === 'file' && e.name.endsWith('.md') && e.name !== INDEX_FILE)
    .map((e) => ({ id: e.name.slice(0, -'.md'.length), sha: e.sha, name: e.name }))
 ...
 */
export async function readMemoryIndex(): Promise<{ body: string; sha: string } | null> {
  const octokit = getOctokit()
...
function renderIndexLine(file: { id: string; meta: MemoryFrontmatter }): string {
...
function buildIndexBody(files: MemoryFile[]): string {
  const sorted = [...files].sort((a, b) => {
    if (a.meta.type !== b.meta.type) {
      return MEMORY_TYPES.indexOf(a.meta.type) - MEMORY_TYPES.indexOf(b.meta.type)
    }
...
export async function writeMemoryFile(opts: WriteOptions): Promise<MemoryFile> {
  ...
  if (!isMemoryType(opts.meta.type)) {
    throw new Error(`Invalid memory type: "${opts.meta.type}". Use one of ${MEMORY_TYPES.join(', ')}.`)
  }
  ...
  if (!refreshed) {
    throw new Error('writeMemoryFile: file was written but could not be re-read')
  }
  return refreshed
}
 ...
 * missing files (no-op).
 */
export async function deleteMemoryFile(octokit: Octokit, id: string): Promise<void> {
