...
export type ActivityCategory =
  | "scheduled" // job-scheduler / worker-scheduler / memorize fan-out (event=schedule)
  | "dispatch" // manual job/worker "Run now" via the kody:control issue
  | "command" // a user @kody command on a real issue/PR (event=issue_comment)
  | "manual" // workflow_dispatch
  | "other"; // push, etc.

export const ACTIVITY_CATEGORY_LABELS: Record<ActivityCategory, string> = {
  scheduled: "Scheduled (jobs/workers)",
  dispatch: "Job/worker dispatch",
