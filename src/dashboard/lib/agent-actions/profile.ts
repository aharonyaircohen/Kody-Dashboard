...
/** Where the agentAction's result lands. `pr` opens a pull request (the
 ...
 * (kody2/src/agent-actions/types.ts) exactly — `{ name, command, args?, env? }`
 ...
 * is just the convenient subset for the common "work an issue" agentAction. */
...
export function contractFor(landing: AgentActionLanding): string {
 ...
 * round-trip an existing agentAction. `prompt` and `shellScripts`/`skills`
 * bodies are filled in by the file layer, not here.
 */
export function fieldsFromProfile(
  slug: string,
  profile: Record<string, unknown>,
): Omit<AgentActionFields, "prompt"> {
