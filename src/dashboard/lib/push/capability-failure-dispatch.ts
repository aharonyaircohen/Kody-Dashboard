 ...
 * @fileType utility
 * @domain kody
 * @pattern capability-failure-inbox-dispatch
 * @ai-summary Server-only helper that turns a failed capability run into an inbox
 *   entry for every operator — so a silent failure stops being silent.
 *
 *   Why here, not in the engine: a scheduled capability tick is fanned out from
 ...
 *
 *   Idempotent: entries carry a deterministic id (`capability-fail:<login>:<capability>:
...
export function buildEntries(
  ...
  for (const rec of failures) {
    ...
    const what = rec.capabilityTitle ?? rec.capability;
    const title = stoppedEarly
      ? `Capability stopped early: ${what}`
      : `Capability failed: ${what}`;
    const snippet = `${who} — ${describeFailure(rec)}`;
    for (const login of operators) {
      entries.push({
        id: `capability-fail:${login}:${rec.capability}:${rec.ts}`,
        login,
        source: "other",
        repoFullName,
        threadType: "Run",
 ...
 */
export async function dispatchCapabilityFailures(
  eventType: string,
  ...
  try {
    ...
    for (const repo of touchedRepos) {
      ...
      if (!bg) {
        logger.warn(
          { event: "capability_failure_no_token", repo: repoFullName },
      ...
      if (operators.length === 0) {
        logger.info(
          ...
          "Capability failed but no operators configured — nothing route",
        );
        continue;
      }
      ...
      try {
        ...
        logger.info(
          {
            event: "capability_failure_inbox_appended",
            added,
    ...
    logger.error(
      ...
      "dispatchCapabilityFailures threw — swallowing so webhook still ACKs",
    );
  }
}
