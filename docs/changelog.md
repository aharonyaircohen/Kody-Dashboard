...
promotes on every release, and that the QA agentResponsibility annotates with verdict
...
QA-agentResponsibility side of this.
...
| **QA marker**                | The authoritative per-PR QA state — none / `🔄` / `✅` / `⚠️` — written by the `qa` agentResponsibility, never by the append handler.                                           | trailing segment of each bullet                                                                              |
...
The QA agentResponsibility may append **exactly one** trailing marker, joined to the bullet
with `·`. The marker is the authoritative QA state for that entry:

| State        | Marker                               | Meaning                                                                                |
| ------------ | ------------------------------------ | -------------------------------------------------------------------------------------- |
| **untested** | _(none)_                             | Merged, never QA'd. The agentResponsibility's queue is "oldest bullet with no marker". |
...
> **Where the marker is written:** the `qa` agentResponsibility writes markers directly via
...
> marker format is owned by the agentResponsibility body, not by dashboard TypeScript.
...
All the QA markers ride along into the versioned section unchanged — the agentResponsibility
...
| [`../.kody/agent-responsibilities/qa/agent-responsibility.md`](../.kody/agent-responsibilities/qa/agent-responsibility.md) | The agentResponsibility that writes QA markers (see [./qa.md](./qa.md)) |
...
merge; the **promote handler** cuts a version on release; the **`qa` agentResponsibility**
...
The QA marker. Once a bullet reads `✅`/`⚠️`, the `qa` agentResponsibility skips it — its
...
markers are a machine-to-machine contract (agentResponsibility writes, agentResponsibility reads); the view
