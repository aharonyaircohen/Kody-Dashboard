...
promotes on every release, and that the QA capability annotates with verdict
...
The QA capability may append **exactly one** trailing marker, joined to the bullet
...
> **Where the marker is written:** the `qa` capability writes markers directly via
...
> marker format is owned by the capability body, not by dashboard TypeScript.
...
All the QA markers ride along into the versioned section unchanged — the capability
...
merge; the **promote handler** cuts a version on release; the **`qa` capability**
...
The QA marker. Once a bullet reads `✅`/`⚠️`, the `qa` capability skips it — its
...
markers are a machine-to-machine contract (capability writes, capability reads); the view
