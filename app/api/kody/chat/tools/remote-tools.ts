...
async function callAgent(
  ...
  try {
    ...
    return { error: err instanceof Error ? err.message : 'Failed to reach remote agent' }
  } finally {
    clearTimeout(timer)
  }
}
  ...
  return {
    remote_exec: tool({
      ...
      inputSchema: z.object({
        command: z.string().min(1).describe('Shell command to run on the remote Mac'),
        cwd: z
          .string()
          .optional()
    ...
    remote_write: tool({
      ...
      execute: async ({ path, content }) => {
        logger.info({ actorLogin, action: 'write', path, bytes: content.length }, 'remote_write')
        return callAgent(cfg.funnelUrl, cfg.key, 'write', { path, content })
      },
    }),

    remote_ls: tool({
      description: "List directory contents on the user's remote Mac dev environment.",
      inputSchema: z.object({
