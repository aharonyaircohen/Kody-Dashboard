    ...
    report_bug: tool({
      ...
      inputSchema: z.object({
        title: z
          .string()
          .min(1)
          .describe('Short bug title, e.g. "[Tasks] Edit dialog crashes on empty priority"'),
        pageUrl: z
          .string()
          .url()
        ...
        browser: z
          .string()
          .optional()
          .describe('Browser / device, e.g. "Chrome", "Mobile Safari (iPhone)"'),
        userRole: z
          .string()
          .optional()
        ...
        assignees: z
          .array(z.string())
          .optional()
          .describe('GitHub logins to assign — only set when the user asks for it.'),
      }),
      execute: async (input) => {
        ...
        try {
          ...
          logger.warn({ err, owner, repo, title: input.title }, 'report_bug failed')
          return {
            error: err instanceof Error ? err.message : 'Failed to create bug issue',
          }
        }
      },
    }),
