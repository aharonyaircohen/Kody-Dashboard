...
export const taskInputSchema = z.object({
  ...
  summary: z
    .string()
    .min(1)
    .describe('1–3 sentence overview of what this task is about and why it matters.'),
  requirements: z
    .string()
    .min(1)
    .describe(
  ...
  acceptanceCriteria: z
    .string()
    .optional()
    .describe('Bullet list of conditions under which the task is "done". Optional but strongly preferred.'),
  additionalContext: z
    .string()
    .optional()
    .describe('Anything else that helps Kody plan and implement: links, prior decisions, constraints. Optional.'),
  assignees: z
    .array(z.string())
    .optional()
    .describe('GitHub logins to assign — only set when the user explicitly asks for it.'),
})
...
> {
  ...
  try {
    ...
    return {
      ...
      url: data.html_url,
      labels,
      assignees: data.assignees?.map((a) => a?.login).filter(Boolean) as string[],
      priority,
      category,
