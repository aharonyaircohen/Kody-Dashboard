 ...
 *   agent/capabilities/commands/executables/instructions/config, plus the operator
...
export function createCompanyTools(ctx: Ctx) {
  ...
  return {
    ...
    export_company: tool({
      description: `Export ${repoRef}'s company bundle — all agent, capabilities, context, commands, executables, instructions, and config — as a portable JSON object the user can save or import into another repo.`,
      inputSchema: z.object({}),
      execute: async () => {
        try {
