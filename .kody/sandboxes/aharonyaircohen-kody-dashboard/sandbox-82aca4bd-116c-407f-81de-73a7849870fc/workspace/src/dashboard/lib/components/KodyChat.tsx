...
export function KodyChat({
  ...
  const [selectedGitHubActionsSandbox, setSelectedGitHubActionsSandbox] =
    useState<LocalSandboxSummary | null>(null);
  const selectedGitHubActionsSandboxId = selectedGitHubActionsSandbox?.id ?? null;
  ...
  const agentList = buildAgentList(
    brainConfigured,
    flyConfigured && !selectedGitHubActionsSandboxId,
    brainFlyChatEnabled,
    chatModels,
  );
  ...
  useEffect(() => {
    ...
    selectedModelId,
    lockedAgentId,
    selectedGitHubActionsSandboxId,
  ]);
  ...
  useEffect(() => {
    setSelectedGitHubActionsSandbox((current) => {
      if (!current) return null;
      return (
        localSandboxes.find(
          (sandbox) =>
            sandbox.id === current.id && sandbox.runtime === "github-actions",
        ) ?? null
      );
    });
  }, [localSandboxes]);

  const selectedTerminalTargetValue = selectedGitHubActionsSandbox
    ? `gha:${selectedGitHubActionsSandbox.id}`
    : activeTerminalValue;

  const useGitHubActionsSandboxForLive = useCallback(() => {
    if (!selectedGitHubActionsSandbox) return;
    setSelectedAgentId("kody-live");
    setSelectedModelId(null);
    setActiveChatMode("ai");
  }, [selectedGitHubActionsSandbox, setActiveChatMode]);
  ...
  const handleCreateSandbox = useCallback(async (
    ...
    try {
      ...
      if (body.sandbox.runtime === "local") {
        setSelectedGitHubActionsSandbox(null);
        handleTerminalSandboxTargetChange(body.sandbox);
      } else {
        setSelectedGitHubActionsSandbox(body.sandbox);
      }
      toast.success(
  ...
  const handleSaveLocalSandbox = useCallback(async () => {
    if (
      activeTerminalTransport.type !== "local" ||
      !activeTerminalTransport.sandboxId
    ) {
  ...
  const handleDeleteLocalSandbox = useCallback(async () => {
    if (
      activeTerminalTransport.type !== "local" ||
      !activeTerminalTransport.sandboxId
    ) {
  ...
  const handleTerminalTargetSelect = useCallback(
    (value: string) => {
      ...
      if (sandbox) {
        setSelectedGitHubActionsSandbox(null);
        handleTerminalSandboxTargetChange(sandbox);
      }
      return;
    }
      ...
      if (sandbox) {
        setSelectedGitHubActionsSandbox(sandbox);
        toast.success(`GitHub Actions sandbox "${sandbox.name}" selected`);
      }
      return;
    }
    setSelectedGitHubActionsSandbox(null);
      ...
      try {
        ...
        const startRes = await fetch(startEndpoint, {
          ...
          body: JSON.stringify({
            ...
            idleExitMs: 5 * 60_000,
            hardCapMs: 30 * 60_000,
            ...(!isFlyRoute && selectedGitHubActionsSandboxId
              ? { sandboxId: selectedGitHubActionsSandboxId }
              : {}),
        ...
        <div className="flex min-h-10 gap-2 items-center">
          ...
          {chatMode === "terminal" && (
            <div className="flex min-w-0 flex-1 items-center gap-1.5">
              <select
                value={selectedTerminalTargetValue}
                onChange={(event) =>
            ...
            {selectedGitHubActionsSandbox && (
              <button
                type="button"
                onClick={useGitHubActionsSandboxForLive}
                className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-md border px-2 text-xs text-foreground transition-colors hover:bg-muted"
                title="Use selected GitHub Actions sandbox in Kody Live"
                aria-label="Use selected GitHub Actions sandbox"
              >
                <Github className="h-3.5 w-3.5" />
                Use
              </button>
            ...
            {!selectedGitHubActionsSandbox &&
              ...
              activeTerminalTransport.sandboxId && (
                  <>
                    <button
                      type="button"
                      onClick={() => void handleSaveLocalSandbox()}
                      disabled={sandboxBusy}
                      className="inline-flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
                      title="Save sandbox"
                    ...
                    <button
                      type="button"
                      onClick={() => void handleDeleteLocalSandbox()}
                      disabled={sandboxBusy}
          ...
          {chatMode === "ai" && selectedGitHubActionsSandbox && (
            <span className="inline-flex min-w-0 max-w-[220px] items-center gap-1.5 truncate rounded-md border bg-background px-2 py-1 text-xs text-muted-foreground">
              <Github className="h-3.5 w-3.5 shrink-0" />
              <span className="truncate">
                GH sandbox: {selectedGitHubActionsSandbox.name}
              </span>
              <button
                type="button"
                onClick={() => setSelectedGitHubActionsSandbox(null)}
                className="shrink-0 rounded-sm text-muted-foreground hover:text-foreground"
                title="Clear GitHub Actions sandbox"
                aria-label="Clear GitHub Actions sandbox"
              >
