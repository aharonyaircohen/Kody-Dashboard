...
export function KodyChat({
  ...
  const handleCreateLocalSandbox = useCallback(async () => {
  ...
  const handleTerminalTargetSelect = useCallback(
    (value: string) => {
      if (value.startsWith("sandbox:")) {
        ...
        const sandbox = localSandboxes.find((candidate) => candidate.id === id);
        ...
        <div className="flex min-h-10 gap-2 items-center">
          ...
          {chatMode === "terminal" && (
            <div className="flex min-w-0 flex-1 items-center gap-1.5">
              ...
              <button
                type="button"
                onClick={() => void handleCreateLocalSandbox()}
