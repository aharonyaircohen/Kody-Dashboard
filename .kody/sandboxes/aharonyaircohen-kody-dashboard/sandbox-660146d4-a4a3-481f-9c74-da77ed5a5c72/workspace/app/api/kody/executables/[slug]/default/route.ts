 ...
 * @ai-summary Set (or clear) this executable as the bare-`@kody` default.
 *   POST { target: "issue" | "pr", clear?: boolean } writes
 *   `defaultExecutable` / `defaultPrExecutable` in kody.config.json — the
...
import { writeDefaultExecutable } from "@dashboard/lib/engine/config";
...
export async function POST(
  ...
  try {
    ...
    recordAudit(req, {
      action: "executable.set_default",
      resource: slug,
      detail: clear
        ? `cleared default ${target} executable`
        : `set ${slug} as default ${target} executable`,
    });

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error("[Executables] Error setting default:", error);
