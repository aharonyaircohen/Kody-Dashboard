...
- **cto — unclear:** dev-ci-health (every 15m) has no state file and produced no output. Additionally, the duty targets the `dev` branch — which does not exist in this repo. Cannot confirm delivery or diagnose whether the duty is failing or the system is not generating state. **Effect:** CI health on `dev` is permanently invisible regardless of cause.
...
- **tech-writer — weak:** docs-code/docs-readme (daily) last produced issues #23/24 on May 30 — four days without output on a daily cadence. **Effect:** documentation gaps go unreported; coverage rot resumes unchallenged.
- **Orphaned duty:** `clear-empty-goals` (every 1d) has no `staff:` field and no owner. It will never run.

- Changes since last week: cto weak→unclear; qa weak→unclear; tech-writer steady→weak.