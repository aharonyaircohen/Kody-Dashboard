...
Implemented the Engine Contract feature defining the convention-based interface between the dashboard and any CI/CD engine (assumed GitHub-only).
...
- **types.ts not modified**: Phase 3 of the implementation plan suggested updating `src/dashboard/lib/types.ts` to import from contracts. However, the existing `KodyPipelineStatus` in types.ts has schema differences from the contract types (e.g., `controlMode` variants differ). Modifying types.ts risks breaking existing code that depends on the original shapes. The contract types are available for new code to use via imports from `@dashboard/contracts`.

- **ESLint config missing**: The `pnpm lint` command fails because there's no `eslint.config.js` file. This is a **pre-existing issue** in the project — lint fails even without my changes. The `pnpm typecheck` passes successfully.
...
- **Labels**: `{engine}:{suffix}` format (building, done, failed, paused, timeout)
- **Status Comment**: `<!-- {engine}-status:{taskId} -->` marker + `<!--pipeline-data\n{json}\n-->` JSON block
- **ETag polling**: 304 Not Modified responses for efficient polling

### Kody Extension

- `KodyPipelineStatus` extends `PipelineStatus` with cost, token usage, actor history, control mode
- `translatePRReviewToAction()` converts PR `changes_requested` reviews to `rerun` action
- Backward compatible — existing Kody engine output passes both generic and Kody schemas
